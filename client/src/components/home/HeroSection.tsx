...
export default function HeroSection() {
  ...
  const onMouseUp = () => { if (!isDragging.current) return; isDragging.current = false; animRef.current = requestAnimationFrame(animate); };
  ...
  const onTouchEnd = () => { isDragging.current = false; animRef.current = requestAnimationFrame(animate); };
    ...
    <div style={{ background: "#ffffff", fontFamily: "'DM Sans','Inter',sans-serif", overflowX: "hidden" }}>
      ...
      {drawerOpen && (
        <div
          ...
          style={{ position: "fixed", inset: 0, background: "rgba(10,22,40,0.4)", zIndex: 200, backdropFilter: "blur(4px)" }}
        />
      )}
      ...
      <div style={{
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "4px", flex: 1 }}>
          ...
          <a href="/tracking" onClick={() => setDrawerOpen(false)}
            style={{ color: "#0a1628", fontSize: "16px", fontWeight: 600, textDecoration: "none", padding: "14px 16px", borderRadius: "12px", display: "flex", alignItems: "center", gap: "8px" }}>
      ...
      <nav style={{ position: "sticky", top: 0, zIndex: 100, display: "flex", alignItems: "center", justifyContent: "space-between", padding: navPadding, height: "70px", background: "rgba(255,255,255,0.9)", backdropFilter: "blur(20px)", borderBottom: "1px solid rgba(10,22,40,0.07)" }}>
        ...
        {(isMobile || isTablet) && (
          <button onClick={() => setDrawerOpen(true)}
            style={{ background: "rgba(10,22,40,0.06)", border: "none", borderRadius: "10px", width: "40px", height: "40px", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: "5px", padding: "10px" }}>
      ...
      <div style={{ position: "relative", zIndex: 1, display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", padding: heroPadding }}>
        ...
        {/* Headline — mobile: 32px, tablet: 48px, desktop: clamp */}
        <h1 style={{
          fontSize: isMobile ? "32px" : isTablet ? "48px" : "clamp(36px,5vw,64px)",
          fontWeight: 800, lineHeight: 1.1,
          letterSpacing: isMobile ? "-0.5px" : "-2px",
          color: "#0a1628", maxWidth: "740px",
          margin: "0 0 16px",
        }}>
          World-Class Logistics<br />For Africa <span style={{ color: "#5b3a8e" }}>&amp; Beyond.</span>
        </h1>

        {/* Subtitle — 16px body, mb: 32px (4×8) */}
        <p style={{
          fontSize: "16px",
          color: "#6b7280",
          maxWidth: "480px",
        ...
        <button style={{
          ...
          fontFamily: "inherit", width: isMobile ? "100%" : "auto", maxWidth: isMobile ? "320px" : "none",
          boxShadow: "6px 6px 16px rgba(10,22,40,0.2),-3px -3px 10px rgba(255,255,255,1)",
      ...
      >
        <div style={{ position: "relative", height: isMobile ? "280px" : "360px", cursor: isDragging.current ? "grabbing" : "grab" }}>
          <svg style={{ position: "absolute", top: 0, left: 0, width: "100%", pointerEvents: "none", zIndex: 2 }}
            height={STRING_Y + ARC_DEPTH + 10} viewBox={`0 0 ${vpWidth} ${STRING_Y + ARC_DEPTH + 10}`} preserveAspectRatio="none">
          ...
          {sets.flatMap(set =>
            SERVICE_CARDS.map((card, i) => {
              const cardW = isMobile ? 180 : CARD_WIDTH;
              const cardStep = isMobile ? 180 + 20 : CARD_STEP;
              ...
              return (
                <div key={`${set}-${i}`} style={{ position: "absolute", left: screenX - cardW / 2, top: 0, width: `${cardW}px`, zIndex: 5, transform: `rotate(${card.rotate}deg)`, transformOrigin: `${cardW / 2}px ${pinHeadCY}px`, pointerEvents: "none" }}>
                  ...
                  <div style={{ position: "absolute", top: cardTop, left: 0, width: `${cardW}px`, background: "#fff", borderRadius: "18px", overflow: "hidden", border: `1px solid ${card.featured ? "rgba(255,140,0,0.28)" : "rgba(10,22,40,0.08)"}`, boxShadow: card.featured ? "0 12px 36px rgba(255,140,0,0.14),0 4px 12px rgba(10,22,40,0.08)" : "0 8px 28px rgba(10,22,40,0.1),0 2px 6px rgba(10,22,40,0.05)" }}>
                    <div style={{ height: isMobile ? "110px" : "170px", overflow: "hidden", position: "relative" }}>
                      <img src={card.image} alt={card.title} style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
                      {card.featured && <div style={{ position: "absolute", top: "10px", right: "10px", background: "#ff8c00", color: "#fff", fontSize: "9px", fontWeight: 700, letterSpacing: "0.6px", padding: "3px 8px", borderRadius: "100px" }}>FEATURED</div>}
                    </div>
                    <div style={{ padding: isMobile ? "10px 12px 12px" : "14px 16px 18px" }}>
                      <div style={{ fontSize: isMobile ? "11px" : "13px", fontWeight: 700, color: "#0a1628", marginBottom: "3px" }}>{card.title}</div>
                      <div style={{ fontSize: isMobile ? "9px" : "11px", color: "#9ca3af" }}>{card.subtitle}</div>
                    </div>
                  </div>
                </div>
              );
      ...
      <div style={{ background: "#f0f2f5", padding: certPadding }}>
        {/* H2 — mobile: 24px (8px grid), desktop: 22px */}
        <h2 style={{
        ...
        {/* Glassmorphism card — overflow hidden clips scrolling logos */}
        <div style={{
          ...
          {isMobile ? (
            // Auto-scroll infinite loop on mobile — fully inside card
            <div style={{ position: "relative", padding: "16px 0", overflow: "hidden" }}>
              ...
              `}</style>
              {/* Left + right fade overlays inside card */}
            ...
            // Desktop/tablet — static row with dividers, inside card with padding
